export const dict = {
  ...
  errors: {
    radio: (label) => [
      ...
      { text: typeof label === "string" ? label.toUpperCase() : label, highlight: true },
  ...
  steps: {
    intro: {
      ...
      hint2Prefix: "Кстати, вы можете настроить внешний вид приложения под себя! Нажмите на ",
    ...
    applicationType: {
      ...
      groupHelper: "подаётся в случае если Вы планируете посетить мероприятие группой (от двух до пяти человек)",
      totalSizeLabel: "Общее количество участников Вашей группы",
      totalSizeHelper: "указывается общее число людей, включая Вас как руководителя группы",
      ...
      unifiedHelper: "условия проживания и обеспечение полностью совпадают с Вашими",
      differentialLabel: "Дифференцированные условия",
      differentialHelper: "индивидуальные предпочтения для каждого из участников Вашей группы",
    },
    personalData: {
      title: "Персональные данные",
      applicantTitleGroup: "2.1. Заявитель",
    ...
    transportation: {
      ...
      toTitle: (step) => `${step}.1. Дорога туда`,
      ...
      fromTitle: (step) => `${step}.2. Дорога обратно`,
      ...
      commentTitle: (step) => `${step}.3. Дополнительно по транспорту`,
    ...
    provisions: {
      title: "Обеспечение",
      foodTitle: (step) => `${step}.1. Продукты питания`,
      foodLabel: "Потребность в питании",
      alcoholTitle: (step) => `${step}.2. Алкогольные напитки`,
    ...
    freeMic: {
      ...
      commentPlaceholder: "Напишите здесь всё, что считаете важным...",
      commentHint: "В этом пункте Вы можете уточнить любые детали или добавить информацию, которая не вошла в предыдущие шаги заявки",
    },
    outro: {
