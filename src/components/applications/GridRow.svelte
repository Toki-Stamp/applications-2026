<script>
  ...
  import { transportMethods, getDayStatus, getMethodStatus } from './gridStore.svelte.js';
...
<tr
  class:hidden-row={filterMode && Object.keys(gridState.activeFilters).length > 0 && !gridState.isRowHighlightedAll(p)}
...
>
  ...
  <td 
    ...
    class:highlighted-col={gridState.isFilterHoveredOrActive('toCity_' + (p.transportTo?.city?.trim().toLowerCase() || ''))}
    onclick={(e) => { 
      e.stopPropagation(); 
    ...
    onmouseenter={() => gridState.hoveredFilter = p.transportTo?.city ? `toCity_${p.transportTo.city.trim().toLowerCase()}` : null}
  ...
  {#each [0, 1, 2] as dayIndex}
    {@render statusCell(`toDay_${dict.options.days[dayIndex]}`, getDayStatus(p.transportTo.day, dict.options.days[dayIndex]))}
  {/each}
  ...
  <td 
    ...
    class:highlighted-col={gridState.isFilterHoveredOrActive(`toTime_${p.transportTo?.time}`)}
    onclick={(e) => {
      e.stopPropagation();
    ...
    onmouseenter={() => gridState.hoveredFilter = p.transportTo?.time ? `toTime_${p.transportTo.time}` : null}
    onmouseleave={() => gridState.hoveredFilter = null}
  >
    {p.transportTo.time || "-"}
  </td>
  ...
  <!-- ОБРАТНО: День -->
  {#each [0, 1, 2] as dayIndex}
    {@render statusCell(`fromDay_${dict.options.days[dayIndex]}`, getDayStatus(p.transportFrom.day, dict.options.days[dayIndex]))}
  {/each}
  ...
  <td 
    class="text-center time-cell interactive-time-cell"
    class:highlighted-col={gridState.isFilterHoveredOrActive(`fromTime_${p.transportFrom?.time}`)}
    onclick={(e) => {
      e.stopPropagation();
    ...
    onmouseenter={() => gridState.hoveredFilter = p.transportFrom?.time ? `fromTime_${p.transportFrom.time}` : null}
    onmouseleave={() => gridState.hoveredFilter = null}
  >
