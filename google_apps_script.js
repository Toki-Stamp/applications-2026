function doPost(e) {
  ...
  try {
    ...
    if (sheet.getLastRow() === 0) {
      sheet.appendRow([
        "Время подачи", "Group ID", "Роль", "Тип заявки", "Условия питания", 
        "Никнейм", "Имя", "Фамилия", "Телефон", 
        "Транспорт ТУДА", "Свободные места", "День выезда", "Время выезда", 
        "Транспорт ОБРАТНО", "День возвращения", "Время возвращения", "Коммент (Транспорт)", 
        "Еда", "Приемы пищи", "Алкоголь", "Приемы алкоголя", 
        "Проживание", "Ночевки", "Коммент (Проживание)", 
        "Свободный микрофон"
      ]);
      sheet.setFrozenRows(1); // Закрепляем шапку
      // Можно также задать стиль шапки (жирный шрифт, фон), если нужно
      sheet.getRange(1, 1, 1, 25).setFontWeight("bold").setBackground("#f3f4f6");
    }
