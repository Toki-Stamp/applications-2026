...
async function verifyPeriodChecked(page, provisionLabel, dayLabel, periodLabel) {
  const provisionItem = page.locator('.provision-item').filter({ hasText: provisionLabel }).first();
  const labelContainer = provisionItem.locator('.label-container').filter({ hasText: dayLabel }).first();
  const cardsGrid = labelContainer.locator('xpath=following-sibling::div[contains(@class, "cards-grid")]');
  const card = cardsGrid.locator('.period-card').filter({ hasText: periodLabel }).first();
...
test.describe('Draft Restoration E2E Tests', () => {
  ...
  test('Exhaustive Draft Check for all steps', async ({ page }) => {
    ...
    await verifyDropdownSelected(page, 'Общее количество участников Вашей группы');
    await verifyRadioChecked(page, 'Условия для участников Вашей группы', 'Дифференцированные условия');
    ...
    await fillGuestText(page, 'Гость #1', 'Номер телефона', '+375 29 999 88 77');

    await page.reload();
    await page.locator('button:has-text("Продолжить")').click();

    await expect(page.locator('.step-indicator').filter({ hasText: 'Шаг 2' })).toBeVisible();
    ...
    await page.reload();
    await page.locator('button:has-text("Продолжить")').click();
    await expect(page.locator('.step-indicator').filter({ hasText: 'Шаг 3' })).toBeVisible();
    ...
    // --- STEP 4 (Provisions) ---
    await selectGuestRadio(page, 'leader_nick', 'Потребность в питании', 'Буду кушать');
    await checkPeriod(page, 'Потребность в питании', 'В пятницу', 'Вечер');
    await selectGuestRadio(page, 'leader_nick', 'Потребность в алкоголе', 'Без алкоголя');
    
    await selectGuestRadio(page, 'guest_name', 'Потребность в питании', 'Без питания');
    await selectGuestRadio(page, 'guest_name', 'Потребность в алкоголе', 'Без алкоголя');

    await page.reload();
    await page.locator('button:has-text("Продолжить")').click();
    await expect(page.locator('.step-indicator').filter({ hasText: 'Шаг 4' })).toBeVisible();

    await verifyGuestRadioChecked(page, 'leader_nick', 'Потребность в питании', 'Буду кушать');
    await verifyPeriodChecked(page, 'Потребность в питании', 'В пятницу', 'Вечер');
    await verifyGuestRadioChecked(page, 'leader_nick', 'Потребность в алкоголе', 'Без алкоголя');
    await verifyGuestRadioChecked(page, 'guest_name', 'Потребность в питании', 'Без питания');
    await verifyGuestRadioChecked(page, 'guest_name', 'Потребность в алкоголе', 'Без алкоголя');
    ...
    await page.reload();
    await page.locator('button:has-text("Продолжить")').click();
    await expect(page.locator('.step-indicator').filter({ hasText: 'Шаг 6' })).toBeVisible();
