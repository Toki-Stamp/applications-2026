...
async function checkGuestPeriod(
  page,
  guestName,
  provisionLabel,
  dayLabel,
...
test.describe("Form E2E Tests", () => {
  ...
  test("Case 1: Minimal Individual Application (Happy Path)", async ({
    page,
  }) => {
    ...
    page.on("console", (msg) => {
      if (msg.text().includes("Submitting payload to Google Apps Script:")) {
        msg.args().forEach(async (arg) => {
          ...
          if (typeof val === "object" && val.applicationType)
            submittedData = val;
        });
      }
    });
  ...
  test("Case 2: Individual Maximum (Driver, Food, Booking)", async ({
    page,
  }) => {
    let submittedData = null;
    page.on("console", (msg) => {
    ...
    expect(submittedData.applicant.provisions.alcoholPeriods).toContain(
      "sat-eve",
    );
  ...
  test("Case 4: Group Differential", async ({ page }) => {
    ...
    expect(submittedData.guests[0].provisions.foodPeriods).toEqual([]);
    expect(submittedData.guests[0].provisions.alcoholPeriods).toContain(
      "sat-eve",
    );
    expect(submittedData.applicant.accommodation.type).toBe("self");
  ...
  test("Case 5: Validation Check (Negative Path)", async ({ page }) => {
    ...
    await clickNext(page);
    await expect(
      page.locator("text=Пожалуйста, выберите один из вариантов"),
    ).toBeVisible();
    await expect(
      page.locator("h2.block-title", { hasText: "Формат участия" }),
    ).toBeVisible();
  });
  ...
  test("Case 6: Clear Form", async ({ page }) => {
    ...
    await expect(
      page.locator("h2.block-title", { hasText: "Вводная информация" }),
    ).toBeVisible();
  ...
  test("Case 7: Group Differential Complex Provisions and Accommodation", async ({
    page,
  }) => {
    let submittedData = null;
    page.on("console", (msg) => {
    ...
    const guest2Group = page
      .locator(".sub-block-card")
      .filter({ hasText: "Гость #2" })
      .first();
    await guest2Group
      .locator(".form-group")
      .filter({ hasText: "Имя" })
    ...
    expect(submittedData.guests[1].provisions.foodPeriods).toContain(
      "sat-morn",
    );
    ...
    expect(submittedData.guests[0].accommodation.type).toBe("booking");
    expect(submittedData.guests[0].accommodation.nights).toContain("fri-sat");
    expect(submittedData.guests[0].accommodation.comment).toBe(
      "Хочу отдельный номер",
    );
    expect(submittedData.guests[1].accommodation.nights).toContain("sat-sun");
  });
  test("Case 8: Switching from Group to Individual should clear guests and not block submission", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await expect(
      page.locator(".block-title", { hasText: "Транспорт" }).first(),
    ).toBeVisible();
  });
});
